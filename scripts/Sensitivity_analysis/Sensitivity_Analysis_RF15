...
top_features_lgb <- importance_lgb$Feature[1:15]
...
# Select the top 15 features based on importance
top_features_xgb <- importance_xgb$Feature[1:15]
...
# Select the top 15 features based on importance
top_features_cat <- importance_cat_df$Feature[1:15]

# Print the top features from CatBoost
print("Top 15 Features from CatBoost:")
...
write.csv(train_data_RF, "outputs/dataRF_train_flitered_15.csv", row.names = FALSE)
write.csv(test_data_RF, "outputs/dataRF_test_flitered_15.csv", row.names = FALSE)

#----------------------Same As XGBoost_model_Training-----------------------------------------
# ------------------------ Load Data ------------------------

# Read the filtered training and testing datasets
train_data_final <- read.csv("outputs/dataRF_train_flitered_15.csv")
test_data_final <- read.csv("outputs/dataRF_test_flitered_15.csv")
...
for(i in seq_along(folds)) {
  ...
  saveRDS(model, file = paste0("outputs/xg_model_fold_rf15", i, ".rds"))
...
best_model_XG_RF_15 <- readRDS("outputs/xg_model_fold_rf153.rds")

# --------------------------bootstrap----------------------------------------
# Define bootstrap function for XGBoost
bootstrap_metrics_XG_RF_15 <- function(data, indices) {
  test_sample <- data[indices, ]  # Resampled test set
  predictions <- predict(best_model_XG_RF_15, as.matrix(test_sample[, features_XG]))
...
bootstrap_metrics_XG_RF_15 <- boot(data = test_data_final, statistic = bootstrap_metrics_XG_RF_15, R = 1000)  # R is the number of resampling iterations
