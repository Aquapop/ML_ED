...
write.csv(training_data, "data107_train.csv")
write.csv(test_data1, "data107_test.csv")
# Exclude the target variable from the training data using Excel and save as train_RF.csv
# Note: This step needs to be performed manually in Excel as per user instructions

# Read the modified training data without the target variable
test6 <- read.csv('train_PMM.csv') 
...
# Function to calculate the mode, ensuring it returns a character value
mode_function <- function(x) {
  ux <- na.omit(x)  # Remove NA values
  if(length(ux) == 0) return(NA_character_)  # Return NA character if no data after removing NAs
  mode_val <- names(sort(table(ux), decreasing = TRUE))[1]  # Calculate and return the mode
  return(mode_val)
}
...
test6 <- test6 %>%
  mutate(across(all_of(factorCols), ~{
    mode_val <- mode_function(.)  # Calculate mode for the current column
    ifelse(is.na(.), mode_val, .)  # Replace NA with mode, keep other values unchanged
  }))

str(test6)

# Load necessary libraries for imputation
library(mice)
library(doParallel)
...
# Function to perform PMM imputation on continuous variables
fix_na <- function(data, m, factorCols) {
  # Initialize the method vector with "" indicating no imputation
  methods <- rep("", ncol(data))
  
  # Set method to "pmm" for numeric variables
  ...
  # Ensure that columns in factorCols are not imputed
  for (col in factorCols) {
    if (col %in% colnames(data)) {
      methods[which(colnames(data) == col)] <- ""
    }
  }
  
  # Perform imputation using the specified methods
  imp <- mice(data, m = m, method = methods, maxit = 5, printFlag = TRUE)
...
# Impute missing values in the training data (test6) with 5 imputations
dataPMM_train <- fix_na(test6, 5, factorCols)
str(dataPMM_train)
write.csv(dataPMM_train, "dataPMM_train.csv")   

# Impute missing values in the testing data

# Save the original testing data to CSV
write.csv(test_data1, "test_PMM.csv")

# Exclude the target variable from the testing data using Excel and save as test_PMM.csv
# Note: This step needs to be performed manually in Excel as per user instructions

# Read the modified testing data without the target variable
dataPMM_test <- read.csv("test_PMM.csv")

# Convert specified columns to factors in the testing data
dataPMM_test <- dataPMM_test %>%
  mutate(across(all_of(factorCols), as.factor))
str(dataPMM_test)

# Impute missing values in categorical variables with the mode in the testing data
dataPMM_test <- dataPMM_test %>%
  mutate(across(all_of(factorCols), ~{
    mode_val <- mode_function(.)  # Calculate mode for the current column
    ifelse(is.na(.), mode_val, .)  # Replace NA with mode, keep other values unchanged
...
# Impute missing values in the testing data with PMM
dataPMM_test <- fix_na(dataPMM_test, 5, factorCols)

# Save the imputed testing data
write.csv(dataPMM_test, "dataPMM_test.csv")  

# Feature Selection

# Read the imputed training and testing data
training_data <- read.csv("dataPMM_train.csv")
test_data1 <- read.csv("dataPMM_test.csv")
...
train_data_PMM <- training_data[, c(selected_features, 'ED')]
test_data_PMM <- test_data1[, c(selected_features, 'ED')]
write.csv(train_data_PMM, "dataPMM_train_filtered.csv")
write.csv(test_data_PMM, "dataPMM_test_filtered.csv")
...
target_col <- "ED" 
features <- setdiff(names(train_data_final), target_col)
best_model_XG_PMM <- readRDS("xg_model_fold_pmm3.rds")

# Define bootstrap function to calculate evaluation metrics
bootstrap_metrics_XG_PMM <- function(data, indices) {
  test_sample <- data[indices, ]  # Resample the test set
  predictions <- predict(best_model_XG_PMM, as.matrix(test_sample[, features]))
...
results_metricsPMM <- boot(data = test_data_final, statistic = bootstrap_metrics_XG_PMM, R = 1000)  # R is the number of resamples
